...
class Application(QMainWindow):
    ...
    def create_home_tab(self):
        ...
        form_layout.addRow(QLabel("Migration Script Time Unit:"), self.migration_time_unit_edit)
        form_layout.addRow(QLabel("Migration Model Type:"), self.migration_model_type_edit)
    ...
    def create_migration_tab(self):
        ...
        # User inputs for Migration Advice
        form_layout = QFormLayout()

        self.migration_time_unit_edit = QLineEdit('20')
        self.migration_model_type_edit = QLineEdit('xgboost')

        form_layout.addRow(QLabel("Script Time Unit:"), self.migration_time_unit_edit)
        form_layout.addRow(QLabel("Model Type:"), self.migration_model_type_edit)

        migration_layout.addLayout(form_layout)

        # Add a plot or DataFrame display
        self.migration_output_label = QLabel("Migration Advice Output")
        migration_layout.addWidget(self.migration_output_label)

        # Start button for migration advice
        start_button_migration = QPushButton("Start Migration Advice")
        start_button_migration.clicked.connect(self.start_migration_advice)
        migration_layout.addWidget(start_button_migration)
    ...
    def start_monitoring(self):
        ...
        self.start_migration_advice()

    def start_migration_advice(self):
        # Migration advice monitoring
        migration_inputs = {
    ...
    def update_migration_plot(self):
        migration_file_path = 'C:\\Users\\B_L_C\\Desktop\\aypos_monitor\\migration_advices\\current_output.csv'
        gain_file_path = 'C:\\Users\\B_L_C\\Desktop\\aypos_monitor\\migration_advices\\gain_output.csv'

        # Load and display migration advice CSV
        if os.path.exists(migration_file_path):
            df = pd.read_csv(migration_file_path)
            self.migration_output_label.setText(df.to_string())  # Display the migration DataFrame

        # Load and display the aypos gain JSON
        if os.path.exists(gain_file_path):
            df_gain = pd.read_csv(gain_file_path)
            # Format the gain DataFrame for display
            gain_output = f"\n\nproposed Gain Data:\n{df_gain.to_string()}"
            self.migration_output_label.setText(
                self.migration_output_label.text() + gain_output)  # Append below migration advice data

        # Update the migration advice output every second
        QTimer.singleShot(1000, self.update_migration_plot)

    def approve_decision(self):
        if self.flag is not None:
            pd.DataFrame({'flag': [self.flag]}).to_csv('ac_command.csv', index=False)

    def approve_preventive_decision(self):
        if self.flag_preventive is not None:
            pd.DataFrame({'flag': [self.flag_preventive]}).to_csv('preventive_ac_command.csv', index=False)
