...
def initialize_confg_data(pm_ip_address, vm_ip_address):
    ...
    physical_machines = [[pms_j['res'][pm]['hypervisor_hostname'], int(pms_j['res'][pm]['vcpus']), int(pms_j['res'][pm]['memory_mb']), int(pms_j['res'][pm]['local_gb']), int(pms_j['res'][pm]['idle consumption'])] for pm in pms_j['res']]


    virtual_machines = {vm : [vm, int(vms_j['res'][vm]['vcpus']), int(vms_j['res'][vm]['ram']), int(vms_j['res'][vm]['disk'])] for vm in vms_j['res']}
...
def migration_advices(data, vm_power):
    ...
    # Karar değişkenleri
    x = LpVariable.dicts("x", ((i, j) for i in range(len(fiziksel_makineler)) for j in range(len(sanal_makineler))), cat="Binary")
    ...
    model += lpSum(y[i] * fiziksel_makineler[i][4] for i in range(len(fiziksel_makineler))) + lpSum(x[i, j] * sanal_makineler[j][4] for i in range(len(fiziksel_makineler)) for j in range(len(sanal_makineler)))
    ...
    for i in range(len(fiziksel_makineler)):
        model += lpSum(x[i, j] * sanal_makineler[j][1] for j in range(len(sanal_makineler))) <= fiziksel_makineler[i][1] * y[i]
        model += lpSum(x[i, j] * sanal_makineler[j][2] for j in range(len(sanal_makineler))) <= fiziksel_makineler[i][2] * y[i]
        model += lpSum(x[i, j] * sanal_makineler[j][3] for j in range(len(sanal_makineler))) <= fiziksel_makineler[i][3] * y[i]
    ...
    for i, fiziksel_makine in enumerate(fiziksel_makineler):
        ...
        if y[i].varValue == 1:
            toplam_tuketim = fiziksel_makineler[i][4] + lpSum(x[i, j].varValue * sanal_makineler[j][4] for j in range(len(sanal_makineler)))
...
def write_to_csv(value, file_path='C:\\Users\\B_L_C\\Desktop\\aypos_monitor\\migration_advices\\migration_advices.csv'):
...
def delete_file_if_exists(file_path='C:\\Users\\B_L_C\\Desktop\\aypos_monitor\\migration_advices\\migration_advices.csv'):
    if os.path.isfile(file_path):
        os.remove(file_path)


file_path = 'C:\\Users\\B_L_C\\Desktop\\aypos_monitor\\migration_advices\\gain_output.csv'


def main_m(user_inputs):
    ...
    write_to_csv(df_migration_advices, 'C:\\Users\\B_L_C\\Desktop\\aypos_monitor\\migration_advices\\migration_advices.csv')
    ...
    gain_dict = gain.to_dict()
    print(gain_dict)

    gain_number = gain_dict["prop_gain"][1]
    prop_power = gain_dict["prop_power"][1]
    cur_power = gain_dict["cur_power"][1]

    thd = {"data": {"gain": gain_number, "pow_prop": prop_power, "power_cur": cur_power}}
    # mig_chart_data["pms"]["gains"] = thd

    print(mig_chart_data)

    response_gain = transact(thd, "http://10.150.1.167:8003/prom/push/migration_text")
    print(response_gain)
    ...
    try:
        ...
        while True:
            connection = connect(p_ip_addresses)
            prime_mig_data, mig_chart_data, gain, df_migration_advices = process(connection, data, actual_df, scaler,
                                                                                 cols,
                                                                                 model, script_time_unit)
            temp_a_process = df_migration_advices
            write_to_csv(df_migration_advices,
                         'C:\\Users\\B_L_C\\Desktop\\aypos_monitor\\migration_advices\\migration_advices.csv')
            write_to_csv(gain, file_path)
            # gain_data = gain.to_json()["ratio"]["1"]
            # ...
